...
(define-language CoreChkC+
  ...
  (e ::= (n : τ) x (let x = e in e) (malloc K ω) (cast τ e)
     (e + e) (& e → f) (* e) (* e = e) ;; (unchecked e) I don't need this one?
     ;; NEW (real):
     (to m e) ;; Do I need to change m to K?
     ;; NEW:
     (if e e e)
     (strlen e)
     ;; (dyn-bound-cast τ e) I can remove this?
     ;; NEW (real):
     (free e)
     ;; NEW functions
     (call n e ...)) ;; Do I need to change n to e?
  ...
  (eE ::= hole
      ...
      (x = eE)
      (eE - ee) (i - eE)
      (* eE) (* eE = ee) (* i = eE) ;; These don't need changing
      (eE <=? ee)
      (i <=? eE)
      (if eE ee ee)
      (strlen eE)
      ;; New (real), so is the above New:
      (free eE)
      (let x = eE in ee)
      (let x = i in eE)

      ;;New for functions
      (call eE ee ...) ;; Is this right?
      (call n n ... eE ee ...))


  ;; erased serious terms
  (eS ::= x (malloc K ee) ;; Does malloc need a K here?
      ...
      ;; New (real):
      (free ee)
      (call n ee ...) ;; Do I need to change n to ee?
      ;; exceptions
      (enull) (ebounds))
...
(define --->^CEK
  (reduction-relation
   ...
   ;; fixed these mallocs, since it now also requires K
   (--> (eH eΣ i ((malloc K []) eK ...))
        (eH eΣ (malloc K i) (eK ...))
        eE-Malloc-Cont)

   (--> (eH eΣ (malloc K eS) (eK ...))
        (eH eΣ eS ((malloc K []) eK ...))
        eE-Malloc-Eval)
...
;; modify the stuff below this so it properly uses K.
...
(define-judgment-form CoreChkC+
  ...
  [(where (maybe-e (ptr K (ntarray le he τ_1))) (⊢ty-env-lookup Γ x_2)) ;; changed m_' to K
   (⊢ty>>> (⊢extend-Γ (x_2 = maybe-e : (ptr K (ntarray le (x_1 + 0) τ_1))) ;;  should I change m to K?
   ...
   (⊢ty>>> Γ σ ρ m (let x_1 = (strlen x_2) in e)
           (let x_1 = (strlen (⊢insert-check #f ρ x_2 x_2 (ptr K (ntarray le he τ_1)))) ;; changed m_' to K
                in (⊢insert-strlen-widening K x_1 x_high ee))
        τ_2)]
  ...
  [(⊢ty>>> Γ σ ρ m e ee (ptr K (struct T))) ;; changed m_' to K
   ...
   ------------- T-Amper
   (⊢ty>>> Γ σ ρ m (& e → f) ((⊢insert-null-check′ ρ ee (ptr K (struct T)))  + n) (ptr K τ_f))]
  ...
  [(⊢wf Γ (ptr t ω)) ;; should I change c to K?
   (where #t (⊢malloc-type-wf ω))
   ------------- T-Mat
   (⊢ty>>> Γ σ ρ m (malloc t (ω + (expt 2 (*N*-1)))) (malloc t (⊢sizeof (ω + (expt 2 (*N*-1))))) (ptr t (ω + (expt 2 (*N*-1)))))]

  [(⊢wf Γ (ptr u ω)) ;; should I change c to K?
   (where #t (⊢malloc-type-wf ω))
   ------------- T-Mau
   (⊢ty>>> Γ σ ρ m (malloc u (ω + (expt 2 (*N*-1)))) (malloc u (⊢sizeof (ω + (expt 2 (*N*-1))))) (ptr u (ω + (expt 2 (*N*-1)))))]
  ...
  [(⊢ty>>> Γ σ ρ m e ee (ptr K ω)) ;; changed m_' to K
   ...
   (where #t (⊢mode-ok? K m))
   ------------- T-Def
   (⊢ty>>> Γ σ ρ m (* e) (⊢widen-bounds-deref ρ e (ptr K ω)
                                           (* (⊢insert-check #f ρ e ee (ptr K ω)))) τ)]
  ...
  ;; for assign, modify it so it has the new bounds check.
  [(⊢ty>>> Γ σ ρ m e_1 ee_1 (ptr K ω)) ;; changed m_' to k
